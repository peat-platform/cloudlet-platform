...
loglet = loglet.child({component: 'type-api'});
   ...
   for ( var i in type['@context']) {
      ...
      if(type['@context'].hasOwnProperty(i)) {
         ...
         if (undefined === entry['@data_type'] || -1 === allowedTypes.indexOf(entry['@data_type'].toLowerCase())) {
            ...
            type_val = entry['@data_type'];

            if ( peatUtils.isTypeId(type_val) ){
               subTypes.push(type_val);
            }
            else{
               errs.push(type_val + ' is not a valid type.');
            }
         }
      }
   }
...
var genPostMessage = function(msg, senderToDao, senderToClient, terminal_handler) {
   ...
   var typeId = 't_' + peatUtils.hash(msg.json);

   var typeRestUrl = "/api/v1/types/" + typeId;
...
var genGetMessage = function(msg, senderToDao, senderToClient, terminal_handler) {
   ...
   if ( msg.path === '/api/v1/types' || msg.path === '/api/v1/types/' ) {
      ...
      if (query.id_only === true) {
         rrd.monitorIncrement('type_get_id_only')
      };
      ...
      var type =   peatUtils.extractTypeId(msg.path);

      if (undefined === type || null === type || "" === type){
         return senderToClient.send(msg.uuid, msg.connId, zmq.status.BAD_REQUEST_400, zmq.standard_headers.json, {'error' : 'Invalid type id' });
      }
...
var genPatchMessage = function(msg, senderToDao, senderToClient, terminal_handler) {
   ...
   for ( var i =0; i < msg.json.length; i++) {
      var entry       = msg[i];
      var typeId      = 't_' + peatUtils.hash(entry);
      var typeRestUrl = "/api/v1/types/" + typeId;
...
var processMongrel2Message = function (msg, senderToDao, senderToClient, terminal_handler,logger) {
