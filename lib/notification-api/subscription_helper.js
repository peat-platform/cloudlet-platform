...
var init = function (config, log) {
   ...
   if (cluster == undefined) {
      cluster = new couchbase.Cluster(config.couchbase_cluster);
   }
   ...
   switch (err.code) {
      ...
      case 12:
         if (0 === documentName.indexOf('t_')) {
            return 'PEAT Type already exists (' + rest_uuid + ').';
         }
...
var sendGCM = function (msg) {
   ...
   var message = {
      ...
      collapse_key   : 'peat_notification',
      tty            : 600,
   ...
   gcm.send(message, function (err, messageId) {
      if (err) {
         logger.error("GCM: Something has gone wrong!");
      }
      else {
         logger.info("GCM: Sent with message ID: ", messageId);
      }
   });
};
...
var checkPermissions = function (msg, callback, success_function) {
   ...
   if (peatUtils.isTypeId(msg.database) || peatUtils.isAttachmentId(peatUtils.extractAttachmentId(msg.database))) {
      ...
      if(token['peat-token-type'] !== undefined && token['peat-token-type'] === 'session'){
         if (msg.json.cloudletid !== undefined && msg.json.cloudletid === token.cloudlet){
            success_function();
            return
         }
         else {
            callback(null, {'error': 'Cannot subscribe to others cloudlets with this token'}, HTTPStatus.INTERNAL_SERVER_ERROR);
         }
      }
      else {
         third_party = msg.token.context;
...
var postSub = function (msg, callback) {
   ...
   if(subscription === null || subscription === ""){
      var error = {'error': 'Invalid subscription JSON'}
      callback(null,error,HTTPStatus.BAD_REQUEST)
   }

   if ( !peatUtils.isCloudletId(subscription.cloudletid) ) {
      error = { 'error': ' \'cloudletid\' is not a valid ID' }
      callback(null, error, HTTPStatus.BAD_REQUEST)
      return
   }
   else if ( !peatUtils.isTypeId(subscription.typeid) ) {
   ...
   checkPermissions(msg, callback, function() {
      ...
      var subId = 's_' + peatUtils.hash(subscription);
...
var getSub = function (msg, callback) {
   ...
   var cloudlet = msg.token.cloudlet;
   var subId = getSubId(msg.path);

   if (getPathSize(msg.path) > 4) {
      var dbName = cloudlet + '+' + subId;
...
var deleteSub = function (msg, callback) {
   dbc.hasMember(msg, 'path');

   var cloudlet = msg.token.cloudlet;
   var subId = getSubId(msg.path);

   if(!peatUtils.isSubscriptionId(subId)){
      callback(null,
         { 'error': 'Error: Incorrect subscription ID' },
         HTTPStatus.BAD_REQUEST)
   }

   var dbName = cloudlet + '+' + subId;

   bucket.remove(dbName, function (err, result) {

      if ( err ) {
         logger.error(err);
         var httpCode = (12 === err.code) ? HTTPStatus.CONFLICT : HTTPStatus.INTERNAL_SERVER_ERROR;
         callback(null,
            { 'error': 'Error: ' + getCouchbaseError(err, dbName, dbName) },
            httpCode)
      }
      else {
         /*var subscription = result.value;
         subscription['_revision'] = result.cas['0'] + '-' + result.cas['1'];*/
         callback(null, {'@id':subId}, HTTPStatus.OK)
      }
   })
};
...
var processMessage = function(msg, senderToDao, senderToClient, config, path) {
   ///console.log('1')
   if (path.indexOf("sse") >= 0) {
      ///console.log('2')
      ...
      jwt.verify(tokenB64, config.trusted_security_framework_public_key, function (err, token) {
         ...
         evaluateMessage(msg, function (err, results, httpStatusCode) {
            if ( err ) {
               ///console.log('13');
               logger.error(err);
            }
            senderToClient.send(msg.uuid, msg.connId, httpStatusCode, zmq.standard_headers.json, results)
         });
