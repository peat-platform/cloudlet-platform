...
var init = function(conf){
   config = conf;
   subscriptions.init(conf.couchbase_cluster)
};
...
var postPermissions = function(msg, errCallback, senderToDao){
   ...
   if(undefined !== permissions.perms) {
      ...
      for (var i in permissions.perms['@service_enabler']){
         ...
         subscriptions.getAppSubscriptions(msg, se.app_id);
      }

      subscriptions.getAppSubscriptions(msg, app_api_key);
...
var processMessage = function(msg, senderToDao, senderToClient, config){
   ...
   jwt.verify(tokenB64, config.trusted_security_framework_public_key, function (err, token) {
      ...
      if ( undefined !== err && null !== err ) {
         ...
         var daoMsg = processMongrel2Message(msg, senderToDao, function (err) {
