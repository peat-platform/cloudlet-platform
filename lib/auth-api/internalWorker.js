...
var processAuthorisations = function (config, intent, msg, senderToClient, senderToDao){
   ...
   {
      var verified_user    = scrypt.verifyKdf(new Buffer(msg.body[0].response.password, 'base64'), intent.data.password);
...
var processSessions = function (config, intent, msg, senderToClient){
   ...
   if(intent.action === 'GENERIC_CREATE')
   {
      ...
      var verified_user = scrypt.verifyKdf(new Buffer(msg.body.response.password, 'base64'), intent.data.password);
