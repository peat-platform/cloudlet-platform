...
var processAuthorisations = function (config, intent, msg, senderToClient, senderToDao){
   ...
   {
      var verified_secret = verifyKdfSync(msg.body[1].response.secret,   intent.data.secret)
      var verified_user   = verifyKdfSync(msg.body[0].response.password, intent.data.password)
...
var verifyKdfSync = function(hash, plain){
   try{
      return scrypt.verifyKdfSync(new Buffer(hash, "base64"), plain)
   }
   catch(e){
      return false
   }
...
var processSessions = function (config, intent, msg, senderToClient){
   ...
   if(intent.action === 'GENERIC_CREATE')
   {
      ...
      var verified_user = verifyKdfSync(msg.body.response.password, intent.data.password)
