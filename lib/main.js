...
var logIncoming = function(logger, msg){
   ...
   //Added special case for Mongrel2 instances that may be behind a haproxy balancer
   if (undefined !== msg.headers['haproxy-forwarded-for']){
      msg.headers['x-forwarded-for'] = msg.headers['haproxy-forwarded-for']
      msg.headers['REMOTE_ADDR']     = msg.headers['haproxy-forwarded-for']
   }
   ...
   if (logger.levels(0) > 20){
      ...
      if (undefined !== msg.headers.authorization && 3 === msg.headers.authorization.split('.').length) {
         var tokenB64 = msg.headers.authorization.replace("Bearer ", "");
         var decoded  = jwt.verify(tokenB64, conf.platform.trusted_security_framework_public_key)

         info.cloudlet        = (undefined === decoded.cloudlet)             ? "-" : crypto.createHash('sha1').update(decoded.cloudlet ).digest('hex');
         info.client_id       = (undefined === decoded.client_id)            ? "-" : crypto.createHash('sha1').update(decoded.client_id).digest('hex');
         info.context         = (undefined === decoded.context)              ? "-" : crypto.createHash('sha1').update(decoded.context  ).digest('hex');
         info.scope           = (undefined === decoded.scope)                ? "-" : decoded.scope;
         info.peattokentype   = (undefined === decoded["peat-token-type"])   ? "-" : decoded["peat-token-type"];
         info.iat             = (undefined === decoded.iat)                  ? "-" : decoded.iat;
      }
...
var cloudletPlatform = function(config){
   ...
   notifApi.init(config.platform, logger);
...
swaggerDef(conf.swagger_def_config,      logger);
swaggerDef(conf.http_swagger_def_config, logger);
